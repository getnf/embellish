...
public class Embellish.Window : Adw.ApplicationWindow {
	...
	private GLib.Settings settings = new GLib.Settings ("io.github.getnf.embellish");
	...
	private Gtk.CustomFilter installed_filter;
	private Gtk.CustomFilter available_filter;
	...
	private void setup_fonts () {
		...
		this.installed_filter = new Gtk.CustomFilter ((obj) => {
			var font = obj as Embellish.Font;
			return font.is_installed;
		});

		this.available_filter = new Gtk.CustomFilter ((obj) => {
			var font = obj as Embellish.Font;
			return !font.is_installed;
		});

		var installed_model = new Gtk.FilterListModel (filtered_model, this.installed_filter);
		var available_model = new Gtk.FilterListModel (filtered_model, this.available_filter);
	...
	private Gtk.Widget create_row (Embellish.Font font) {
		...
		if (font.description != null) {
			var description = font.description.strip ();
			row.set_subtitle (Markup.escape_text (_(description)));
		}
	...
	private Gtk.Box create_suffix (Font font) {
		...
		var action_button = create_install_button (font, box);
		box.append (action_button);

		return box;
	}

	private Gtk.Widget create_install_button (Font font, Gtk.Box parent_box) {
		var button = new Gtk.Button ();
		button.add_css_class ("flat");

		if (font.is_installed) {
			button.set_child (new Gtk.Image.from_icon_name ("embellish-remove-symbolic"));
			button.set_tooltip_text (_("Uninstall"));
		} else {
			button.set_child (new Gtk.Image.from_icon_name ("embellish-download-symbolic"));
			button.set_tooltip_text (_("Install"));
		}

		button.clicked.connect (() => {
			// Show spinner while operating
			button.set_child (new Adw.Spinner ());
			button.set_tooltip_text (null);
			button.set_sensitive (false);

			if (font.is_installed) {
				ulong handler_id = 0;
				handler_id = library.uninstallation_completed.connect ((f, success, error_msg) => {
					if (f != font)return;
					library.disconnect (handler_id);
					font.is_installed = !success;
					if (success) {
						button.set_child (new Gtk.Image.from_icon_name ("embellish-download-symbolic"));
						button.set_tooltip_text (_("Install"));
						toast_overlay.add_toast (new Adw.Toast (_("%s uninstalled").printf (font.display_name)));
					} else {
						button.set_child (new Gtk.Image.from_icon_name ("embellish-remove-symbolic"));
						button.set_tooltip_text (_("Uninstall"));
						toast_overlay.add_toast (new Adw.Toast (_("Failed to uninstall %s: %s").printf (font.display_name, error_msg)));
					}
					button.set_sensitive (true);
					this.installed_filter.changed (Gtk.FilterChange.DIFFERENT);
					this.available_filter.changed (Gtk.FilterChange.DIFFERENT);
				});
				library.uninstall.begin (font);
			} else {
				ulong handler_id = 0;
				handler_id = library.installation_completed.connect ((f, success, error_msg) => {
					if (f != font)return;
					library.disconnect (handler_id);
					font.is_installed = success;
					if (success) {
						button.set_child (new Gtk.Image.from_icon_name ("embellish-remove-symbolic"));
						button.set_tooltip_text (_("Uninstall"));
						toast_overlay.add_toast (new Adw.Toast (_("%s installed").printf (font.display_name)));
					} else {
						button.set_child (new Gtk.Image.from_icon_name ("embellish-download-symbolic"));
						button.set_tooltip_text (_("Install"));
						toast_overlay.add_toast (new Adw.Toast (_("Failed to install %s: %s").printf (font.display_name, error_msg)));
					}
					button.set_sensitive (true);
					this.installed_filter.changed (Gtk.FilterChange.DIFFERENT);
					this.available_filter.changed (Gtk.FilterChange.DIFFERENT);
				});
				library.install.begin (font);
			}
		});
